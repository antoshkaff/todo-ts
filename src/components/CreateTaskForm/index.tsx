...
import { useTaskActions } from '@/context/TaskContext.tsx';
import { type Task } from '@/types/task.ts';
import { generateUUID } from '@/utils/generateUUID.ts';

type OnChangeFn = (event: React.ChangeEvent<HTMLInputElement>) => void;
type OnSubmitFn = (event: React.FormEvent<HTMLFormElement>) => void;
type CreateTaskFn = (taskContent: string) => Task;
...
const CreateTaskForm = () => {
    ...
    const createTask: CreateTaskFn = (taskContent) => {
        return {
            id: generateUUID(),
            content: taskContent,
            isFinished: false,
    ...
    const onSubmit: OnSubmitFn = (event) => {
        ...
        addTask(createTask(taskContent));
    };

    const onChange: OnChangeFn = (event) => {
        const input = event.target;
        setTaskContent(input.value);
    };
