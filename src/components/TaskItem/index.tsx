...
import { BASE_TRANSITION, LINE_VARIANTS } from '@/constants/motionPresets.ts';
...
const TaskItem = ({ isChecked, taskEntity }: TaskProps) => {
    const { deleteTask, finishTask } = useTaskActions();

    const onChange: React.ChangeEventHandler<HTMLInputElement> =
        useCallback(() => {
            finishTask(taskEntity.id);
        }, [finishTask, taskEntity.id]);

    const onDelete = useCallback(() => {
        deleteTask(taskEntity.id);
    }, [deleteTask, taskEntity.id]);
    ...
    return (
        <article className={styles.task}>
            ...
            <span
                className={clsx(styles.content, isChecked && styles.finished)}
            >
